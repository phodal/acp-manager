...
/**
 * Registers agent management tools with an MCP [Server].
 *
 * This exposes agent lifecycle and coordination tools over the Model Context Protocol,
 * allowing any MCP-compatible client (e.g., Claude, Cursor, VS Code) to manage agents.
 *
 * Tools registered:
 * 1. `list_agents` — List all agents in the workspace
 * 2. `create_agent` — Create a new agent
 * 3. `get_agent_status` — Get detailed agent status
 * 4. `get_agent_summary` — Get agent work summary
 * 5. `read_agent_conversation` — Read agent conversation history
 * 6. `send_message_to_agent` — Send inter-agent messages
 * 7. `delegate_task` — Delegate a task to an agent
 * 8. `report_to_parent` — Report completion to parent agent
 * 9. `wake_or_create_task_agent` — Wake or create an agent for a task
 * 10. `send_message_to_task_agent` — Send message to task's assigned agent
 * 11. `subscribe_to_events` — Subscribe to workspace events
 * 12. `unsubscribe_from_events` — Unsubscribe from workspace events
 *
 * Usage:
 * ```kotlin
 * val mcpServer = Server(...)
 * AgentHubToolManager(agentTools, "my-workspace").registerTools(mcpServer)
 * ```
 */
class AgentHubToolManager(
    private val agentTools: AgentTools,
    private val defaultWorkspaceId: String,
) {

    /**
     * Register all agent management tools with the MCP server.
     */
